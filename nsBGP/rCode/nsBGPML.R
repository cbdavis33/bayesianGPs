...
sim_data_model <- stan_model(file = 'nsBGP/stanCode/generateDataNSBGP.stan')
...
dat_list <- list(n = 40, D = d, mu = mu, sigmaEps = sigmaEps, 
                 rho = array(rho, dim = d),
...
comp_gp_mod_ML <- stan_model(file = 'nsBGP/stanCode/nsBGPML.stan')
...
parsToMonitor <- c("mu", "rho","sigmaEps", "muV", "rhoV", "sigmaV")
